...
export const updateFollower: RequestHandler = async (req, res) => {
    ...
    if (!isValidObjectId(profileId)) return res.status(422).json({
        error:
            "Invalid profile id!"
    })
...
export const getAutoGeneratedPlaylist: RequestHandler = async (req, res) => {
    ...
    if (result) {
        await Playlist.updateOne({ owner: req.user.id, title },
            { $set: { title, item: result.items, visibility: "auto" } },
            { upsert: true }
        )
    }
...
export const getFollowersProfile: RequestHandler = async (req, res) => {
    ...
    const [result] = await User.aggregate([
        { $match: { _id: req.user.id } },
        {
            $project: {
                followers: {
                    $slice: ["$followers", parseInt(pageNo) * parseInt
                        (limit), parseInt(limit)]
                }
            }
        },
...
export const getFollowingsProfile: RequestHandler = async (req, res) => {
    ...
    const [result] = await User.aggregate([
        { $match: { _id: req.user.id } },
        {
            $project: {
                followings: {
                    $slice: ["$followings", parseInt(pageNo) * parseInt
                        (limit), parseInt(limit)]
                }
            }
        },
...
export const getPrivatePlaylistAudios: RequestHandler = async (req, res) => {
    ...
    const aggregationLogic = [
        {
            $match: {
                _id: new Types.ObjectId(playlistId), owner: req.user.id
            }
        },
        {
            $project: {
